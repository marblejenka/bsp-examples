...
public class MaximumValueVertexWriter extends
		VertexWriter<MaximumValueVertex, Text, Text> {
	@Override
	public OrbContext<Text, Text> vertexWrite(MaximumValueVertex vertex) {
		...
		orbContext.write(new Text(vertex.getVertexID()),
				new Text(Integer.toString(vertex.getMaxValue())));
		return orbContext;
	}
}